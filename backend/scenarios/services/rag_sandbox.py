...
def retrieve_documents(scenario, message, submitted_documents, defenses):
    ...
    for doc in seed_docs:
        content = doc.content
        doc_events = detect_external_instruction(content) + detect_flag_leak(content, source="rag_doc")
        ...
        retrieved.append({
            "title": doc.title,
            "content": content[:700],
            "isPoisoned": bool(defenses.get("instructionDataSeparation") and doc_events),
    ...
    for index, item in enumerate(submitted_documents or []):
        ...
        retrieved.append({"title": title, "content": content[:700], "isPoisoned": bool(defenses.get("instructionDataSeparation") and doc_events), "metadata": {"source": "submitted"}})
